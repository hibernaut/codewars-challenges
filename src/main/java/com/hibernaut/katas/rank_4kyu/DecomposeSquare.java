 ...
 * Note that there are actually two ways to
 * decompose 11², 11² = 121 = 1 + 4 + 16 + 100 = 1² + 2² + 4² + 10²
...
public class DecomposeSquare {
    public static String decompose(long n) {
        return recursiveDecomposition(n - 1, n * n);
    }

    private static String recursiveDecomposition(long number, long remain) {
        String temp = null;
        for (; number > 0; number--) {
            long difference = remain - number * number;
            ...
            if(difference == 0) {
                ...
                temp = recursiveDecomposition(Math.min(number - 1, (long) Math.sqrt(difference)), difference);
