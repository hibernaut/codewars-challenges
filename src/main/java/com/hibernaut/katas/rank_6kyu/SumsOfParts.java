...
public class SumsOfParts {
    public static int[] sumParts(int[] ls) {
        ...
        //Calculate first sum which includes all numbers and add it to the sums list
        int sum = Arrays.stream(ls)
                .reduce(0, Integer::sum);
        sums.add(sum);

        //Iterate over the array, subtract element from previous sum and add it to the sum list
        for (int l : ls) {
            sum -= l;
            sums.add(sum);
        }
