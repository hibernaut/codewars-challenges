 ...
 * less than or equal to the given limit t, if that sum exists, or otherwise nil, null, None, Nothing,
...
public class BestTravel {
    public static Integer chooseBestSum(int t, int k, List<Integer> ls) {
        ...
        for (int i = 0; i < ls.size(); i++) {
            if(ls.get(i) <= t) {
                if(k == 1) {
                    bestSum = Math.max(bestSum, ls.get(i));
                }

                else {
                    temp = chooseBestSum(t - ls.get(i), k - 1, ls.subList(i + 1, ls.size()));

                    if(temp != null) {
                        bestSum = Math.max(bestSum, ls.get(i) + temp);
                    }
                }
            }
        }
