...
/**
 * Create a class called Warrior which calculates and keeps track of their level and skills,
 * and ranks them as the warrior they've proven to be.
 * <p>
 * Business Rules:
 * <p>
 ...
 * A warrior's experience is cumulative, and does not reset with each rise of level. The only exception is when the warrior reaches level 100,
 ...
 * If the warrior's level does not meet the minimum level requirement, the warrior doesn not receive
...
public class TheGreatestWarrior {
    ...
    private String[] ranks = {"Pushover", "Novice", "Fighter", "Warrior", "Veteran", "Sage", "Elite", "Conqueror", "Champion", "Master", "Greatest"};
    private String rank;
    private List<String> achievements;


    /**
     * Initialize beginner warrior
     */
    ...
    /**
     * Change current experience according to the additional experience
     * @param additionalExperience
     */
    private void changeExperience(int additionalExperience) {
    ...
    /**
     * Create training between warriors
     * @param battleDescription
     * @param earnedExperience
     * @param minimumWarriorLevel
     * @return battle description
     */
    public String training(String battleDescription, int earnedExperience, int minimumWarriorLevel) {

        if (this.level >= minimumWarriorLevel) {
            this.changeExperience(earnedExperience);
            this.achievements.add(battleDescription);
        } else {
    ...
    /**
     * Create battle between warriors
     * @param enemyLevel
    ...
    public String battle(int enemyLevel) {
        ...
        if (enemyLevel > 100 || enemyLevel < 1) {
            battleResult = "Invalid level";
        } else if (this.level - enemyLevel > 1) {
            battleResult = "Easy fight";
        } else if((this.level - enemyLevel) == 1) {
            this.changeExperience(5);
            battleResult = "A good fight";
        } else if(this.level == enemyLevel) {
            this.changeExperience(10);
            battleResult = "A good fight";
        } else if(((enemyLevel / 10 - this.level / 10) == 1 && (enemyLevel - this.level) < 5) ||
                ((enemyLevel / 10 == this.level / 10) && (enemyLevel > this.level))) {
            this.changeExperience(20 * (enemyLevel - this.level) * (enemyLevel - this.level));
            battleResult = "An intense fight";
        } else if ((enemyLevel / 10 - this.level / 10) >= 1 && (enemyLevel - this.level) > 4) {
            battleResult = "You've been defeated";
        }
