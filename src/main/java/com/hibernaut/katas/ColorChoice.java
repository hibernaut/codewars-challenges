 ...
 * all having the same design each in 2 colors. Posters must all have a different color combination and you have the choice of 4 colors: red, blue, yellow, green. How many colors can you choose for each poster?
 * The answer is two since 4 choose 2 = 6. The combinations will be: {red, blue}, {red, yellow}, {red, green},
 ...
 * will have a different combination of 3 colors or 5 colors. You will take 3 colors because it's less expensive.
 *
 *  Hence the problem:
 ...
 * n choose x = m (1)
 * for a given m and a given n; m >= 0 and n > 0. If many x are solutions give as result the smallest x.
...
public class ColorChoice {
    public static long checkchoose(long m, int n) {
        ...
        while ((result.compareTo(BigInteger.valueOf(m)) != 0) && (n > x)) {
            x++;
            result =  factorial(n).divide(factorial(x).multiply(factorial(n - x)));
        }

        if (result.compareTo(BigInteger.valueOf(m)) == 0) {
            return x;
        }
